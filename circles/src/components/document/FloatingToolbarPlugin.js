...
export const AiAutoCompleteCheckButton = ({ onClick, isCheck, ...props }) => {
    ...
    return (
        <Tooltip label={`${isCheck ? "Accept" : "Remove"} AI text completion`} aria-label="A tooltip" borderRadius="50px">
            <Button
            ...
            >
                {/* 9756ff c710ff */}
                <Icon width={iconSizePx} height={iconSizePx} color={isCheck ? "#2af310" : "#f72626"} as={isCheck ? HiCheck : HiX} zIndex="10" />
            </Button>
        </Tooltip>
    );
};
...
export const setFloatingElemPosition = (targetRect, floatingElem, anchorElem, verticalGap = VERTICAL_GAP, horizontalOffset = HORIZONTAL_OFFSET) => {
...
const TextFormatFloatingToolbar = ({ editor, anchorElem, isLink, isBold, isItalic, isUnderline, isCode, isStrikethrough, onClose }) => {
