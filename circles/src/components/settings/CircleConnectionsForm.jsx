                ...
                >
                    <HStack marginRight="13px">
                        ...
                        {isSendingRequests ? (
                            ...
                            <Text>{i18n.t(`[${circle.type}] Send request to connect`)}</Text>
                        )}
                    </HStack>
                </Button>
...
export const CircleConnectionsForm = ({ circle, isUpdateForm }) => {
    ...
    const receivedConnectionRequests = useMemo(() => {
        return connections.filter(
            (x) =>
                x.target.id === circle.id &&
                (x.type === "connected_mutually_to_request" || x.type === "admin_by_request")
        );
