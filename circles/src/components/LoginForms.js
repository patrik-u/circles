...
import ScopeContext from "./UserContext";
...
export const LoginRegisterMenu = ({ gsiScriptLoaded, isSigningIn, isSignedIn, satelliteMode }) => {
    const user = useContext(UserContext);
    const isMobile = useContext(IsMobileContext);
    const { isOpen: forgotPasswordIsOpen, onOpen: forgotPasswordOnOpen, onClose: forgotPasswordOnClose } = useDisclosure();
    ...
    const [hasRenderedButton, setHasRenderedButton] = useState(false);
    ...
    }, [isSigningIn, isSignedIn, signInOnClose, signUpOnClose]);

    // useEffect(() => {
    //     if (!gsiScriptLoaded || user || hasRenderedButton) return;

    //     // google sign in script is loaded and user isn't signed in
    ...
    //         logo_alignment: "left",
    //     });
    //     setHasRenderedButton(true);
    // }, [gsiScriptLoaded, user, isMobile, hasRenderedButton, signInButton]);
    ...
    }, [signInButton, signInButton2, isMobile]);
    ...
    return (
        <Box display={user || isSigningIn ? "none" : "block"} marginTop={isMobile ? "15px" : "0px"}>
            <HStack>
                <Button onClick={onSignInClick} rounded={"full"}>
                    {i18n.t("Log in")}
                </Button>
                <Button onClick={onSignUpClick} rounded={"full"}>
                    {i18n.t("Sign up")}
                </Button>
            </HStack>

            {/* Modal popup - Log in */}
            <Modal initialFocusRef={signInInitialRef} isOpen={signInIsOpen} onClose={signInOnClose} closeOnOverlayClick={false} size="sm">
                <Formik
            ...
            {/* Modal popup - Sign up */}
            <Modal initialFocusRef={signUpInitialRef} isOpen={signUpIsOpen} onClose={signUpOnClose} closeOnOverlayClick={false}>
                <Formik
                    ...
                    onSubmit={async (values, actions) => {
                        try {
                            ...
                            let message = error.code === "auth/email-already-in-use" ? i18n.t("emailAlreadyInUse") : "";
                        ...
                        <Form>
                            ...
                            <ModalContent borderRadius="25px">
                                                ...
                                                {({ field, form }) => (
                                                    <FormControl isInvalid={form.errors.firstname && form.touched.firstname}>
                                                        ...
                                                        <Text position="absolute" right="0px" top="5px" fontSize="12px" color="#bbb">
                                                            {form?.values?.firstname ? form.values.firstname.length : 0} / 50
                                                        </Text>
                                                        <InputGroup>
                                                            <Input {...field} id="firstname" ref={signUpInitialRef} type="text" maxLength="50" />
                                            ...
                                            <Field name="lastname">
                                                {({ field, form }) => (
                                                    <FormControl isInvalid={form.errors.lastname && form.touched.lastname}>
                                                        ...
                                                        <Text position="absolute" right="0px" top="5px" fontSize="12px" color="#bbb">
                                                            {form?.values?.lastname ? form.values.lastname.length : 0} / 50
                                                        </Text>
                                                        <InputGroup>
                                                            <Input {...field} id="lastname" type="text" maxLength="50" />
                                        ...
                                        <Field name="password">
                                            {({ field, form }) => (
                                                <FormControl isInvalid={form.errors.password && form.touched.password}>
                                                    ...
                                                    <Text position="absolute" right="0px" top="5px" fontSize="12px" color="#bbb">
                                                        {form?.values?.password ? form.values.password.length : 0} / 200
                                                    </Text>
                                                    <InputGroup>
                                                        <Input {...field} id="password" maxLength="200" type={showPassword ? "text" : "password"} />
                                                        <InputRightElement marginRight={!form.errors.password && form.touched.password ? "30px" : "0px"}>
                                                            <Image
                                                                as={showPassword ? BiHide : BiShow}
                                                                width="23px"
                                                                height="23px"
                                        ...
                                        <Field name="password2">
                                            {({ field, form }) => (
                                                <FormControl isInvalid={form.errors.password2 && form.touched.password2}>
                                                    ...
                                                    <Text position="absolute" right="0px" top="5px" fontSize="12px" color="#bbb">
                                                        {form?.values?.password2 ? form.values.password2.length : 0} / 200
                                                    </Text>
                                                    <InputGroup>
                                                        <Input {...field} id="password2" maxLength="200" type={showPassword ? "text" : "password"} />
                                                        <InputRightElement marginRight={!form.errors.password2 && form.touched.password2 ? "30px" : "0px"}>
                                                            <Image
                                                                as={showPassword ? BiHide : BiShow}
                                                                width="23px"
                                                                height="23px"
                                ...
                                <ModalFooter>
                                    ...
                                    <Button variant="ghost" borderRadius="25px" onClick={signUpOnClose} isDisabled={isSubmitting} lineHeight="0">
                                        {i18n.t("Cancel")}
                                    </Button>
                                </ModalFooter>
                            </ModalContent>
            ...
            <Modal initialFocusRef={forgotPasswordInitialRef} isOpen={forgotPasswordIsOpen} onClose={forgotPasswordOnClose} closeOnOverlayClick={false}>
                <Formik
                    initialValues={{ email: "" }}
                    onSubmit={async (values, actions) => {
                        try {
                        ...
                        <Form>
                            ...
                            <ModalContent borderRadius="25px">
                                        ...
                                        <Field name="email">
                                            {({ field, form }) => (
                                                <FormControl isInvalid={form.errors.email && form.touched.email}>
                                                    ...
                                                    <InputGroup>
                                                        <Input {...field} id="email" ref={forgotPasswordInitialRef} type="text" maxLength="50" />
                                                        {!form.errors.email && form.touched.email && (
                                                            <InputRightElement children={<CheckIcon color="green.500" />} />
                                                        )}
                                                    </InputGroup>
                                ...
                                <ModalFooter>
                                    ...
                                    <Button variant="ghost" borderRadius="25px" onClick={forgotPasswordOnClose} isDisabled={isSubmitting} lineHeight="0">
                                        {i18n.t("Cancel")}
                                    </Button>
                                </ModalFooter>
                            </ModalContent>
