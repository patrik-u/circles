...
import { filteredCirclesAtom, circleAtom } from "components/Atoms";
//#endregion
...
export const CircleHolon = ({ width, height }) => {
    ...
    useEffect(() => {
        ...
        let root = { name: circle.name, children: [{ name: circle.name, children: [] }] };

        // create graph data from circles
        filteredCircles?.forEach((c) => {
            let child = { name: c.name, size: 1, children: [] };
            root.children[0].children.push(child);
        });

        // we can filter graph data to only include those which has this as a parent

        setGraphData(root);
    }, [filteredCircles, circle?.id, circle?.name]);
    ...
    if (!circle || !graphData) return null;

    return <HolonMap width={width} height={height} data={graphData} />;
