...
export const HolonMap = ({ data, width, height }) => {
    ...
    const createChart = useCallback(
        (container) => {
            ...
            let fontsize = d3.scaleOrdinal().domain([1, 3]).range([24, 16]);
    ...
    useEffect(() => {
        if (svgRef.current) {
            d3.select(svgRef.current).selectAll("*").remove(); // cleanup any previous graphs
            createChart(svgRef.current);
        }
    }, [createChart]);

    return <div style={{ width: width + "px", height: height + "px" }} ref={svgRef}></div>;
};
