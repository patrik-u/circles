    ...
    const FilterButton = ({ label, isActive, ...props }) => (
        <Tooltip label={getFilterTooltipLabel(label)} aria-label="A tooltip">
            <Button
                ...
                colorScheme="blue"
                onClick={() => setCircleTypeFilter(label.toLowerCase())}
                {...props}
            >
                {label}
            </Button>
        ...
        >
            ...
            <HStack spacing={4} mb={4}>
                ...
                <CategoryButton
                    label="Similar"
                    icon={MdCompare}
                    isActive={circlesFilter?.categories?.includes("similar")}
                />
                <CategoryButton
                    label="Connected"
                    icon={MdLink}
                    isActive={circlesFilter?.categories?.includes("connected")}
                />
                <CategoryButton
                    label="Mentioned"
                    icon={MdChat}
                    isActive={circlesFilter?.categories?.includes("mentioned")}
                />
                <CategoryButton
                    label="Search"
                    icon={MdSearch}
                    isActive={circlesFilter?.categories?.includes("search")}
                />
            </HStack>
            <Flex flexDirection="row" align="center" mb={"0px"}>
                ...
                <FilterButton
                    label="All"
                    isActive={!circlesFilter?.types}
                    borderRadius="7px 0px 0px 0px"
                    borderRightWidth="0px"
                />
                <FilterButton
                    label="Users"
                    isActive={circlesFilter?.types?.includes("user")}
                    borderRadius="0px"
                    borderRightWidth="0px"
                />
                <FilterButton
                    label="Circles"
                    isActive={circlesFilter?.types?.includes("circle")}
                    borderRadius="0px"
                    borderRightWidth="0px"
                />
                <FilterButton
                    label="Events"
                    isActive={circlesFilter?.types?.includes("event")}
            ...
            <Flex flexDirection="row" align="center">
                <ButtonGroup isAttached>
                    ...
                    <FilterButton
                        label="Documents"
                        isActive={circlesFilter?.types?.includes("document")}
