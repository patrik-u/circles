...
import { userAtom, signInStatusAtom, isMobileAtom } from "@/components/Atoms";
...
import { AiPromptNode } from "@/components/document/AiNodes";
...
import { toISODate } from "@/components/Helpers";
        ...
        <Flex minHeight="100vh" width={"100%"} flexGrow="0" flexShrink="0" overflow="hidden">
            ...
            {!condensed && !isMobile && (
                <Box
                    ...
                    minWidth={documentExplorerWidthPx}
                    height="100%"
            ...
            <Flex
                ...
                width={isMobile ? "100%" : `calc(100% - ${documentExplorerWidth + rightPanelWidth}px)`}
                maxWidth="100%"
            >
                {/* Toolbar */}
                <Box flexBasis="45px" height="45px" minHeight="45px" minWidth="100px" width="100%" flexDirection="row">
                    <Flex
                        width={
                            isMobile
                                ? "100%"
                                : `calc(100% - ${documentExplorerWidth + rightPanelWidth + navigationMenuWidth}px)`
                        }
                        align="center"
        ...
        getDocumentSubscription = onSnapshot(doc(db, "circles", document.id), (doc) => {
            if (doc.exists()) {
                ...
                if (newDocument?.version > document?.version) {
                    console.log(
                        "updating document from server, document?.version= " +
                            document?.version +
                            ", newDocument?.version =" +
                            newDocument?.version
                    );
