            ...
            case "connected_to":
                ...
                return (
                    <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                        <strong>{source.name}</strong> {i18n.t("follows")} <strong>{target.id === user.id ? i18n.t("you") : target.name}</strong>
                    </Text>
                );
            case "connected_mutually_to":
                ...
                if (target.id === user.id) {
                    if (source.type === "user") {
                        ...
                        return (
                            <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                {i18n.t(`Your request to connect to [${source.type}]`)} <strong>{source.name}</strong> {i18n.t(`has been approved`)}
                            </Text>
                        );
                    }
                } else {
                    if (source.type === "user") {
                        return (
                            <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                <strong>{source.name}</strong> {i18n.t(`[user] is now connected to the [${target.type}]`)} <strong>{target.name}</strong>
                            </Text>
                        );
                    } else {
                        return (
                            <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                <strong>{source.name}</strong> {i18n.t(`is now connected to the`)} {i18n.t(target.type)} <strong>{target.name}</strong>
                            </Text>
                        );
                    }
                }
            ...
            case "connected_mutually_to_request":
                ...
                if (source.type === "user" && target.type !== "user") {
                    return (
                        <>
                            <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                <strong>{source.name}</strong> {i18n.t(`[user] has requested to connect to [${target.type}]`)} <strong>{target.name}</strong>
                            </Text>
                            <ApproveDenyButtons />
                        </>
                    );
            ...
            case "admin_by_request":
                ...
                if (source.type === "user" && target.type !== "user") {
                    return (
                        <>
                            <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                <strong>{source.name}</strong> {i18n.t(`[user] has requested to be admin of [${target.type}]`)} <strong>{target.name}</strong>
                            </Text>
                            <ApproveDenyButtons />
                        </>
                    );
                    ...
                    if (target.id === user.id) {
                        return (
                            <>
                                <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                    {i18n.t(`You've been invited to administrate the ${source.type}`)} <strong>{source.name}</strong>
                                </Text>
                                <ApproveDenyButtons />
                            </>
                        );
                        ...
                        return (
                            <>
                                <Text className="circle-list-title" lineHeight="20px" fontSize="16px" align="left">
                                    <strong>{target.name}</strong> {i18n.t(`has been invited to administrate`)} <strong>{source.name}</strong>
                                </Text>
                                <ApproveDenyButtons />
                            </>
                        );
...
const Notifications = ({ satelliteMode, circle, setCircle }) => {
    ...
    useEffect(() => {
        ...
        const notificationsQuery = query(collection(db, "notifications"), where("user_id", "==", user.id), orderBy("date", "desc"), limit(25));
