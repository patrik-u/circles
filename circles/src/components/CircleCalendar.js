...
import { Box, ButtonGroup, Flex, Text, Icon, Button, IconButton, Popover, PopoverTrigger, PopoverContent, PopoverArrow, HStack } from "@chakra-ui/react";
...
const EventHeader = ({ event }) => {
    ...
    return (
        <Popover trigger="hover" gutter="0" isLazy>
            <PopoverTrigger>
                <Flex key={event.id} align="center" flexDirection="row" cursor="pointer" onClick={() => openAboutCircle(event, setToggleAbout)} gap="2px">
    ...
    const renderCells = () => {
        ...
        for (let hour = 0; hour < 24; hour++) {
            for (let day = 0; day < 7; day++) {
                ...
                cells.push(
                    <Box w="14%" p={1} border="1px" borderColor="gray.200" key={day + "-" + hour} overflowY="auto" height="60px" backgroundColor="white">
                        {dayEvents?.map((event) => (
        ...
        return (
            <>
                <Flex justify="space-between" position="relative">
                    ...
                    <IconButton aria-label={`Next ${view}`} icon={<ChevronRightIcon />} onClick={nextPeriod} marginRight="40px" />
                </Flex>
