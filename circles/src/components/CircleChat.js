...
const linkifyMarkdown = (input) => {
    ...
    for (const match of matches) {
        ...
        const precedingText = input.slice(
            Math.max(0, match.index - 50),
            match.index
        ); // arbitrary lookbehind, adjust as necessary
        const followingText = input.slice(
            match.lastIndex,
            Math.min(input.length, match.lastIndex + 50)
        ); // arbitrary lookahead, adjust as necessary
...
export const CircleChatWidget = () => {
    ...
    useEffect(() => {
        ...
        if (chatCircle) {
            ...
            log(
                "Chat circle set to: " + JSON.stringify(circle, null, 2),
                0,
                true
            );
    ...
    useEffect(() => {
        ...
        if (circle?.chat_circle_ids?.length > 0) {
            ...
            let relationSetIds = circle.chat_circle_ids.map((id) =>
                getSetId(user.id, id)
            );

            // subscribe to chat circles
            const chatCirclesQuery = query(
                collection(db, "circles"),
                where(documentId(), "in", relationSetIds)
            );
            const unsubscribeGetChatCircles = onSnapshot(
                chatCirclesQuery,
                (snap) => {
                    ...
                    if (circle?.default_chat_id) {
                        let defaultSetId = getSetId(
                            user.id,
                            circle.default_chat_id
                        );
                        setChatCircle(
                            circles.find((x) => x.id === defaultSetId)
                        );
                    }
                }
            );
        ...
        >
            ...
            {chatCircles.length > 0 && (
                <Box pl={2} pointerEvents="auto" zIndex="10" marginBottom="5px">
                    <Tooltip
                        label={`Chat with ${circle?.name} members`}
                    ...
                    {chatCircles.map((item) => (
                        <Tooltip
                            key={item.id}
                            label={`Chat with ${
                                getRelevantCircle(user, item)?.name
                            }`}
                            aria-label="A tooltip"
                        >
                            <IconButton
    ...
    return (
        <>
            {messages?.map((item) => (
                <Box
                    key={item.id}
                    alignSelf={item.type === "date" ? "center" : "auto"}
                >
                    {item.type === "date" && (
                        <Box
...
const MessageInputBox = ({
    ...
    return (
        <>
            ...
            >
                {isMentioning && (
                    <CircleMention onMention={onMention} query={mentionQuery} />
                )}
                <AutoResizeTextarea
                    ref={textAreaRef}
                    id="message"
...
export const CircleChat = ({ circle }) => {
    ...
    const [mentionedCircles, setMentionedCircles] =
        useAtom(mentionedCirclesAtom);
    ...
    const chatSession = useMemo(
        () => chatData?.sessions[chatData?.sessions.length - 1],
        [chatData]
    );
    ...
    useEffect(() => {
        ...
        let unsubscribeGetChat = onSnapshot(
            doc(db, "chat", circle.id),
            (doc) => {
                ...
                if (!doc.exists || !newChatData) {
                    ...
                    axios
                        .post(`/chat_session`, { circle_id: circle.id })
                        .catch((err) => {
                            console.error(err);
                        });
                    return;
    ...
    useEffect(() => {
        ...
        if (circle?.id) {
            ...
            axios
                .put(`/chat_notifications`, { circle_id: circle?.id })
                .catch((err) => {
                    console.error(err);
                });
        }
    ...
    useEffect(() => {
        ...
        setIsLoadingMessages(true);
        const unsubscribeGetChatMessages = onSnapshot(
            chatMessagesQuery,
            (snap) => {
                ...
                const newMentions = allMentions.filter(
                    (mention) =>
                        !mentionedCircles.find((x) => x.id === mention.id)
                );
    ...
    useEffect(() => {
        ...
        if (
            !circle.is_public &&
            !isConnected(userData, circleId, ["connected_mutually_to"])
        ) {
    ...
    useEffect(() => {
        ...
        for (var message of unfilteredChatMessages) {
            ...
            let formattedMessage = message.has_links
                ? linkifyMarkdown(message.message)
                : message.message;
    ...
    const sendMessage = async (messageValue) => {
        ...
        if (!isEditingMessage) {
            setUnfilteredChatMessages([
                ...unfilteredChatMessages,
                newChatMessage,
            ]);
        ...
        if (isEditingMessage) {
            ...
            try {
                postMessageResult = await axios.put(
                    `/chat_messages/${messageToEdit.id}`,
                    {
                        message: formattedMessage,
                    }
                );
    ...
    const confirmDeleteMessage = async () => {
        ...
        try {
            deleteMessageResult = await axios.delete(
                `/chat_messages/${messageToDelete.id}`
            );
        } catch (err) {
            console.error(err);
        }
    ...
    const transformMessageWithMentions = (rawMessage) => {
        ...
        mentionsList.forEach((mention) => {
            const markdownLink = `[${mention.name.slice(
                1
            )}](codo.earth/circles/${mention.id})`; // remove the '@' from the mention name
            ...
            >
                ...
                >
                    {!isAuthorized && (
                        ...
                        >
                            <Text>
                                {i18n.t(
                                    `You need to join the [${circle?.type}] to chat`
                                )}
                            </Text>
                        </Box>
                    )}
                    {!circle?.is_public && (
                        ...
                        >
                            ...
                            <Box flexGrow="1" />
                            <AboutButton
                                circle={getRelevantCircle(user, circle)}
                            />
                            <Tooltip
                                label={
                ...
                >
                    ...
                    <ModalContent borderRadius="25px">
                        <ModalHeader>
                            {i18n.t("Delete the message")}
                        </ModalHeader>
                        <ModalCloseButton />
                        <ModalBody>
                            <Text fontSize="18px">
                                {i18n.t("Do you want to delete the message?")}
                            </Text>
                        </ModalBody>
